...
namespace ItemChecklist.Possession
{
    ...
    {
        public static readonly PossessionView Empty = new PossessionView(new Dictionary<int, int>(), new HashSet<int>(), new Dictionary<long, int>());
        ...
        public int CountSkin(int objectId, int skinIndex) => _aux.TryGetValue(DiscoveredState.PackKey(objectId, skinIndex), out var c) ? c : 0;

        // Cattle colours + placed paintable furniture (secondDim = variation).
        public int CountColour(int objectId, int variation) => _aux.TryGetValue(DiscoveredState.PackKey(objectId, variation), out var c) ? c : 0;
    }
}
