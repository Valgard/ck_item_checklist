...
| Class | Responsibility |
|---|---|
| `PossessionScanner` | Resolves the inventory world (`World.All`, max `ContainedObjectsBuffer` count = ServerWorld in SP), then scans **all** placed `ObjectDataCD` entities. **Possession = carried + base storage.** Carried = the player's whole `ContainedObjectsBuffer` (always live; includes the 0–9 equipment slots). Base storage = placed furniture/display within `AnchorRadius` of a **workbench-anchored** station; contents read from the entity buffer. **Iter-31: anchors = workbenches + the `CraftingCD` stations standing within `AnchorRadius` of a workbench; the link is workbench→station only (never station→station), a single workbench suffices, and the old ≥2-station `ClusteredAnchors` filter is DELETED — see the workbench-anchor paragraph below + `docs/gotchas.md § Possession Base Scope`.** Counts the placed object itself, not just contents. **Reads the universally-needed `ObjectDataCD` + `LocalTransform` in bulk via `ToComponentDataArray` (chunk-sequential memcpy, index-aligned with `ToEntityArray`) — not a per-entity `GetComponentData` random-chunk lookup; per-entity `em` access stays only for the player + the gated near-anchor minority (Iter-27 perf, sandbox-safe, scan MAX 21.5→9.6ms). See `docs/gotchas.md § ECS Scan Performance` + `docs/iteration-history.md § Iter-27`.** **Iter-28: three count paths — (1) carried, (2) container contents (`AddBuffer`), (3) the placed object itself (`AddOne`). World nature is excluded from path #3 ONLY (`!PossessionClassifier.IsWorldNature`), so chest-stored nature (path #2) + carried still count and the remember-from-afar feature is preserved. See `docs/gotchas.md § Possession Persistence & World Nature`.** **Iter-42: the one-time `ledger.PruneByPredicate` eviction that used to run at the top of `Scan` is REMOVED — its `WorldNaturePruned` gate was never serialized, so it ran on every world load, and an id-predicate sweep cannot tell path #3 (evict) from path #2 (keep), so it deleted chest-stored nature; loaded far from base that loss was visible and got persisted. The path-#3 write gate + the `v2→v3` discard migrations make it unnecessary. See `docs/gotchas.md § A "one-time" cleanup gated on an unpersisted flag`.** **Iter-44: the scanner hands the whole snapshot to `ledger.ApplyScan` in ONE call — the two per-tile accumulators, the container-tile set, the player position, its own `WithinAnchor` gate as a delegate, and the grace flag — and gets back everything that was removed. It no longer decides anything: the rules moved into the ledger, where the "would the scan have seen that tile" predicate exists once instead of being duplicated between the caller and the prune. It also scans at most **once per frame** (`Update` has two call sites with no return between them, and the ledger's miss rule counts scans, so the same read twice looked like two independent observations). Cattle colour aux is keyed to ONE deterministic anchor tile per scan (`CattleAuxTile`, the lowest packed anchor key), not the anchor nearest the animal — Iter-41's version hopped as animals wandered, measured at ~12 tiles added/removed per save interval, which broke the Iter-31 save-write-skip and double-counted colours under Iter-44's miss delay.** |
| `PossessionLedger` | Keys containers by world tile `(x,z)` (packed `long`), merges `carried + per-container` in `BuildView`, and marks items present only in not-currently-observed containers as "remembered" (check ownership while away from base). **Iter-42: the Iter-28 `PruneByPredicate(Func<int,bool>)` + its `WorldNaturePruned` flag are GONE — a per-id sweep over this dict cannot distinguish a placed wild object from the same id stored in a chest (one flat per-tile dict serves both scan paths), so it destroyed real possession on every load. The invariant that holds after Iter-42 is narrower than "two removers" (which Iter-43 corrected — it was wrong): **no path deletes from this ledger by id-predicate any more.** The paths that DO remove remembered state, as of **Iter-44**: `ApplyScan` — the single destructive entry point, which merges every observed tile and then prunes — and `LoadFrom`, which `Clear()`s unconditionally and discards the whole file on a version mismatch. Inside `ApplyScan` a removal needs the streaming grace to have passed AND either a container observed on that tile (its buffer is authoritative) or the tile to be one the scan WOULD have seen anything on (`ScanWouldSeeTile`: within `PruneRadius` 48 of the player AND anchor-covered — the Iter-41 two-condition premise, now defined once and read by both the merge and the prune). A count that is merely LOWER applies at once; an entry that is ABSENT needs either a confirmed absence or the same tile to have been unconfirmed-absent on the immediately preceding scan too ("one miss is not evidence", per key for the merge and per tile for the prune). Iter-43's three writers `SetLiveContainer`/`SetLiveAux`/`ClearAux` and Iter-44's first-draft `BeginScan`/`Publish`/`PruneStaleNear` protocol are all GONE — a multi-call protocol cannot be enforced at compile time in this language subset, so with one entry point "no scan is open" and "the prune was skipped" stop being representable.** **Iter-45: a tile has THREE dimensions, not two — `stored` (path #2, a container's contents), `placed` (path #3, the object standing there) and `aux`. Paths #2 and #3 shared one dict from Iter-20 to Iter-44, which is the missing provenance Iter-42 was about. `placed` shrinks on the tile-scope premise only (it IS the entity, seen exactly when the tile is in scope), so it behaves like `aux`; only `stored` keeps the "a container was observed here" disjunct, because a buffer we read is evidence about its own contents from any distance. The reverse index (`TilesHolding`/`CountTilesHolding`) counts BOTH — every provenance is a real location an arrow can point at — while `CountContainerTilesHolding` backs the tooltip's "in N chests" wording, which before the split was asserted about placed objects too.** **Iter-31/41/45 — the file format: `Serialize` writes a first-line marker (`v2` Iter-31 → `v3` Iter-41's aux segment → `v4` Iter-45's placed segment) plus, since Iter-45, a `#n=<tiles>` comment line so a file cut exactly at a line boundary is not read as a valid shorter one. The marker is an ACCEPTED SET (v4 and v3), which makes Iter-45 the first schema change here that migrates rather than discards: `placed` is the LAST segment, so v3's three fields keep their meaning and one parser reads both shapes — three segments means a tile whose stored/placed split is still the v3 assumption, and it is written back three-segment until an observation corrects it. Every earlier marker is still discarded once (the cluster-anchor-polluted pre-v3 ledger), after which the base re-scans clean.** |
| `PossessionStore` | Persists the ledger per character GUID via `API.ConfigFilesystem` (hand-rolled ASCII, sandbox-safe). **Iter-31: a 64-bit FNV-1a content hash of the last-written text per GUID elides the disk write (and the `DirectoryExists` probe) when the freshly serialized ledger is unchanged — the `WriteCharacter` hook fires every autosave but base storage rarely changes between two, and the Wine `Write` is the 5–13 ms cost. The first save per character always lands (no `_lastSavedHash` entry); the hash is recorded only after a successful write → a skip can never drop a needed save. FNV-64 (not 32-bit `GetHashCode`) — see `docs/gotchas.md § Hash width is the safety knob`.**  **Iter-43: `Load` reports a `StoreLoadStatus` (`NoFile`/`Loaded`/`Failed`) — previously every outcome returned a bare empty ledger indistinguishably, so a transient read fault let the next autosave persist emptiness over an intact file (the first save of a launch always lands: the hash cache is per-session and an empty ledger cannot hash-match a populated one). `Failed` is set BEFORE anything else can throw, because `LoadFrom` clears its dicts before parsing and a mid-parse throw would otherwise look like success. A version-marker discard is reported (byte count + actual first line) but is NOT a failure — it is the intended migration and must stay writable. `PetCollectionStore` mirrors all of this and is the unrecoverable store (ever-owned set, no second source).** |
| `PossessionIncidentStore` (Iter-43) | Durable, **NOT diagnostics-gated** reporting for events that can cost remembered possession: `load-failed`, `ledger-discarded`, `shrink`. Persists to `mods/ItemChecklist/possession-incidents.txt` via `API.ConfigFilesystem`, modelled on `PhantomViolationStore` (same rationale, one tier more serious — that one guards an unreachable catalog row, this one the player's data). Ungated on purpose: `Diagnostics` defaults off, and a data-loss report that only appears once someone already suspected a problem reports nothing when it matters. Deduped by a caller-supplied key (so a structural fault records once, not per autosave), capped at 200 lines (the Iter-28 unbounded-store lesson), timestamps from `Time.realtimeSinceStartup` — `System.DateTime` is unproven sandbox surface and not worth a whole-mod compile failure for a log field. |
| `ModConfig` (root `ItemChecklist` namespace, `unity/ItemChecklist/ModConfig.cs`) | Exposes five **live Mod Settings Menu settings** — `Enabled` (master on/off toggle, default on — off makes the mod fully inert: no possession scan, the window will not open, the HUD stays hidden), `Mode` (Iter-36 `CounterMode` {Discovery, Possession}), `AnchorRadius` (Slider, 16–96, default 48), `ScanIntervalSeconds` (Iter-38; Choice over preset seconds {1,2,3,5,8,10,15,20,25,30}, default 3) and (Iter-30) `Diagnostics` — registered in `ItemChecklistMod.Init` (call order = render order), read live each scan/save, persisted via the framework's CoreLib `ConfigFile` (`mods/ItemChecklist/config.cfg`). `Diagnostics` (default off) gates the per-scan/per-save timing logs + the one-time counted-object dump. Replaced the former `API.Config` surface; renamed from `PossessionConfig` + hoisted to root for cross-mod uniformity (every sibling mod exposes a root-namespace `ModConfig`). |
